...
def query_bloodhound_api(uri: str, method: str, creds: dict,
                         body_file_name: str = "", body_json: dict | None = None) -> dict:
    ...
    headers = {
        "User-Agent":    "bloodhound-ce-ctl/0.1.0",
    ...
    try:
        ...
        raise RuntimeError(f"Cannot reach BloodHound at {creds['bh_url']} — is it running? ({e.__cause__ or e})") from None
...
def wait_for_ready(since: datetime, container: str = DOCKER_CONTAINER, timeout: float = 60.0) -> None:
    ...
    proc = subprocess.Popen(
        ["docker", "logs", container, "--follow", "--since", since_str],
...
def open_browser(bh_url: str) -> None:
    for query in (USER_QUERY, DA_QUERY):
        url = bh_url.rstrip("/") + query
        try:
            subprocess.Popen(
                ["xdg-open", url],
            ...
            webbrowser.open(url)  # non-Linux fallback
...
def poll_job(creds: dict, job_id: int, interval: float) -> None:
    ...
    while True:
        ...
        if status == 2:
            log.info("Job %d completed successfully. Opening browser...", job_id)
            open_browser(creds["bh_url"])
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Python based CLI manager for BloodHound Community Edition")
    parser.add_argument("-u", "--url",       default="http://localhost:8080", help="BloodHound URL")
    parser.add_argument("-i", "--tokenid",   default=os.environ.get("BH_TOKEN_ID"),  help="BloodHound Token ID (default: $BH_TOKEN_ID)")
    parser.add_argument("-k", "--tokenkey",  default=os.environ.get("BH_TOKEN_KEY"), help="BloodHound Token Key (default: $BH_TOKEN_KEY)")
    ...
    parser.add_argument("-p", "--poll",      action="store_true", help="Poll upload job status and open browser on completion")
    parser.add_argument(      "--container", default=DOCKER_CONTAINER, help=f"Docker container name (default: {DOCKER_CONTAINER})")
    ...
    if args.clear:
        try:
            jobs_exist = has_upload_jobs(creds)
        ...
        if not jobs_exist:
            ...
            if upload_requested:
                try:
                    wait_for_ready(since=clear_time, container=args.container)
                except Exception as e:
                    print(f"[!] {e}")
                    sys.exit(1)
    ...
    if upload_requested:
        ...
        if args.poll and last_job_id is not None:
            poll_job(creds, last_job_id, POLL_INTERVAL)
